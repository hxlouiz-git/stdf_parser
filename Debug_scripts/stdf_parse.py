"""
STDF parser built on stdf_parser (pip package).
...
from stdf_parser.RECFuncs import (
    get_headers,
    PTR, MPR, FTR,
    PIR, PRR,
    MIR, MRR,
...
# ── record-type / record-subtype dispatch table ────────────────────────────────
...
def _record_key(rec) -> tuple[int, str]:
    """Canonical key for a test result record."""
...
def _register_test_meta(tests: dict, rec) -> TestMeta:
    ...
    if existing is None:
        ...
        # Patch only fields present in this (possibly full) record
        if scal is not None:
...
def parse(path: str) -> StdfData:
    ...
    # current wafer ID for each head (set from WIR, cleared on WRR)
    ...
    while pos + 4 <= total:
        ...
        if parser is None:
            continue  # unknown or intentionally skipped record
        ...
        if (rec_typ, rec_sub) == (1, 10):    # MIR
            ...
            meta = _register_test_meta(data.tests, rec)
            col = _record_key(rec)
            # RTN_RSLT is an array of per-pin results; scale each element
            try:
                scal = meta.res_scal
